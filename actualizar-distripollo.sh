#!/bin/bash
# ════════════════════════════════════════════════════════════════
# 🐔 DISTRIPOLLO - Actualizar schema + cargar productos
# ════════════════════════════════════════════════════════════════
# 1. Agrega precio mayorista a productos
# 2. Crea tabla Mayorista con CC/NIT
# 3. Carga los 90+ productos de Distripollo
# ════════════════════════════════════════════════════════════════
...
echo ""
echo -e "${BLUE}🐔 Actualizando Distripollo...${NC}"
echo ""

if [ ! -f "package.json" ] || [ ! -f "prisma/schema.prisma" ]; then
    echo -e "${RED}❌ No estás en el proyecto distripollo-bot${NC}"
    echo -e "${YELLOW}   Corre: cd ~/Desktop/distripollo-bot${NC}"
    exit 1
fi

# ─── 1. Actualizar schema de Prisma ───
echo -e "${YELLOW}📝 Actualizando schema (agregando mayoristas + precios duales)...${NC}"
...
echo -e "${YELLOW}🗄️  Aplicando cambios a Supabase...${NC}"
npx prisma generate
npx prisma db push --accept-data-loss
echo -e "${GREEN}✓ Base de datos actualizada${NC}"
echo ""

# ─── 3. Seed con TODOS los productos de Distripollo ───
echo -e "${YELLOW}🌱 Cargando los 90+ productos de Distripollo...${NC}"
...
echo -e "${GREEN}✓ Seed actualizado con todos los productos${NC}"
echo ""

# ─── 4. Ejecutar el seed ───
echo -e "${YELLOW}🚀 Cargando productos a Supabase...${NC}"
npm run db:seed
echo ""

echo -e "${BLUE}╔══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  ✅ DISTRIPOLLO ACTUALIZADO                     ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${YELLOW}Lo que tienes ahora:${NC}"
echo -e "  ✓ Base de datos con ${GREEN}precios duales${NC} (detal + mayorista)"
echo -e "  ✓ Tabla ${GREEN}Mayorista${NC} (con CC/NIT)"
echo -e "  ✓ ${GREEN}74 productos${NC} cargados (campesinos, congelados, precocidos, etc)"
echo -e "  ✓ Zonas (Centro, Norte, Sur, Oriente, Occidente)"
echo ""
echo -e "${YELLOW}Para ver los productos:${NC}"
echo -e "  ${GREEN}npm run db:studio${NC}"
echo -e "  Abre la tabla ${BLUE}Producto${NC}"
echo ""
echo -e "${YELLOW}Próximos pasos:${NC}"
echo -e "  1. Subes el logo de Distripollo"
echo -e "  2. Yo creo el panel admin con colores correctos"
echo -e "  3. Actualizamos el bot para:"
echo -e "     • Entender texto pegado"
echo -e "     • Identificar mayoristas por CC/NIT"
echo -e "     • Usar lenguaje natural"
echo ""
